...
public class ClientGameManager implements Runnable {
    ...
    //receiveMove nunca se usa, solo se utiliza en el metodo getReceiveMove, metodo que tampoco se usa nunca
    ...
    private void playGame() {
    	// Remove setup panel
        Platform.runLater(() -> BoardScene.getRootPane().getChildren().remove(BoardScene.getSetupPanel()));
                ...
                if(Game.getMove().isAttackMove()) {
            		...
            		Platform.runLater(() -> {
            			try {
                            ...
                            //Eliminamos el codigo duplicado extrayendo ese codigo a un metodo nuevo aparte(setFade)
    ...
    //Metodo getReceiveMove() nunca se usa
    ...
    private void setFade(FadeTransition fade){
    ...
    /* Metodo que no estoy seguro
    PARA ELIMINAR EL CODIGO DUPLICADO DE LAS LINEAS 223-254

    private int increaseOrDecreaseShift(int moveX, int moveY, int shiftX, int shiftY){
        if(moveX > 0) {return shiftX = 1;}
        else if(moveX < 0) {return shiftX = -1;}
        else if(moveY > 0) {return shiftY = 1;}
        else if(moveY < 0) {return shiftY = -1;}
    }*/
